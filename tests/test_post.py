...
class TestPostAPI:
    ...
    def test_posts_auth_get(self, user_client, post, another_post):
        ...
        self.check_post_data(test_post, "GET-запрос к `/api/v1/posts/`", db_post)
    ...
    def test_post_get_current(self, user_client, post):
        ...
        test_data = response.json()
        self.check_post_data(test_data, "GET-запрос к `/api/v1/posts/{id}/`", post)
    ...
    def test_post_change_auth_with_valid_data(self, user_client, post, http_method):
        ...
        response = request_func(f"/api/v1/posts/{post.id}/", data=self.VALID_DATA)
